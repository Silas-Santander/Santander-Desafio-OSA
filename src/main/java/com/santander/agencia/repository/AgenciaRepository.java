...
public interface AgenciaRepository extends JpaRepository<Agencia, Long> {
    ...
    @Query(value = """
        ...
        LIMIT :limite
        """, nativeQuery = true)
    List<Object[]> buscarAgenciasProximasComDistancia(@Param("posX") Double posX,
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    boolean existeAgenciaProxima(@Param("posX") Double posX,
                                 @Param("posY") Double posY,
                                 @Param("distanciaMinima") Double distanciaMinima);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT COUNT(a) FROM Agencia a")
    long countComLock();
